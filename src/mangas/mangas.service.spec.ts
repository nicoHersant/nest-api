...
import { MangasService, Manga } from './mangas.service';
...
describe('MangasService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should combine genre and status filters', () => {
      ...
      expect(result.data.every((m) => m.status === 'ongoing')).toBe(true);
    });
    ...
    it('should persist the new manga to storage', () => {
      ...
      expect(storageService.write).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          expect.objectContaining({ title: 'Naruto', author: 'Masashi Kishimoto' }),
        ]),
      );
    });
